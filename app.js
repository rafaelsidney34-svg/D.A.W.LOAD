...
function loadDatabase() {
  // 1. CARREGAMENTO RÃƒÆ’Ã‚ÂPIDO (CACHE-FIRST)
  ...
  // 2. SINCRONIZAÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã†â€™O EM TEMPO REAL COM FIREBASE
...
// FunÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes de Gerenciamento de Compras Simuladas
...
function addPurchaseId(idString) {
  ...
  if (idString === 'all-paid') {
      idsToProcess = productsData.filter(p => p.category !== "GrÃƒÆ’Ã‚Â¡tis").map(p => p.id).join(',');
  }
...
const topMembers = [
  { nome: "Philippe Romain", icone: "ÃƒÂ¢Ã‚Â­Ã‚Â", tipo: "membro" },
  { nome: "JosÃƒÆ’Ã‚Â© Vieira", icone: "ÃƒÂ¢Ã‚Â­Ã‚Â", tipo: "membro" },
  { nome: "VANDERLEI Cruz", icone: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Ëœ", tipo: "top" },
  { nome: "AMIRRUL HAFIZ", icone: "ÃƒÂ¢Ã‚Â­Ã‚Â", tipo: "membro" },
  { nome: "Eduardo Porcho", icone: "ÃƒÂ¢Ã‚Â­Ã‚Â", tipo: "membro" },
  { nome: "Viktor Stos", icone: "ÃƒÂ¢Ã‚Â­Ã‚Â", tipo: "membro" },
  { nome: "Daniel Rios", icone: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Ëœ", tipo: "top" },
  { nome: "Cleiton Pacheco Gino", icone: "ÃƒÂ¢Ã‚Â­Ã‚Â", tipo: "membro" },
  { nome: "AndrÃƒÆ’Ã‚Â© Sampaio", icone: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Ëœ", tipo: "top" },
  { nome: "Darin Costa", icone: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Ëœ", tipo: "top" },
  { nome: "Steve Landry", icone: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Ëœ", tipo: "top" }
];

const topWeekMembers = topMembers.filter(m => m.tipo === "top");
const regularMembers = topMembers.filter(m => m.tipo !== "top");
const regularMemberIcons = ["ÃƒÂ¢Ã‚ÂÃ‚Â¤", "ÃƒÂ¢Ã‚Â­Ã‚Â", "ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¹", "ÃƒÂ°Ã…Â¸Ã¢â€žÂ¢Ã…â€™", "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥", "ÃƒÂ¢Ã…â€œÃ‚Â¨", "ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â§", "ÃƒÂ°Ã…Â¸Ã…Â¡Ã¢â€šÂ¬", "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã¢â€žÂ¢"];

// ==========================================================
//  LÃƒÆ’Ã¢â‚¬Å“GICA PRINCIPAL ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â DOMContentLoaded
// ==========================================================
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCartUI() {
    ...
    if (cartItemsContainer) {
      ...
      if (cart.length === 0) {
        cartItemsContainer.innerHTML = '<div class="cart-empty-msg">Seu carrinho estÃƒÆ’Ã‚Â¡ vazio.</div>';
      ...
      cart.forEach((item, index) => {
        ...
        div.innerHTML = `
          ...
          <button class="cart-item-remove" data-index="${index}" title="Remover">ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¢</button>
        `;
  ...
  // ==========================================================
  //  AUTH ÃƒÂ¯Ã‚Â¿Ã‚Â½ Login, Registro, SessÃƒÂ¯Ã‚Â¿Ã‚Â½o
  // ==========================================================
  function updateHeaderForUser() {
    ...
    if (user) {
      ...
      if (userGreetingName) userGreetingName.textContent = `ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ‚Â¤ ${user.name.split(" ")[0]}`;
    } else {
      clearPurchases();
  ...
  loginForm?.addEventListener("submit", function (e) {
    ...
    if (result.success) {
      ...
      updateHeaderForUser();
      renderProducts();
      showToast(`Bem-vindo de volta, ${result.user.name.split(" ")[0]}! ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¹`);
    } else {
  ...
  registerForm?.addEventListener("submit", function (e) {
    ...
    if (password !== passwordConfirm) {
      if (registerError) { registerError.textContent = "As senhas nÃƒÆ’Ã‚Â£o coincidem."; registerError.classList.add("show"); }
      return;
    }
    ...
    if (result.success) {
      ...
      updateHeaderForUser();
      renderProducts();
      showToast(`Conta criada com sucesso! Bem-vindo, ${result.user.name.split(" ")[0]}! ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Â°`);
    } else {
  ...
  if (btnHeaderLogout) {
    btnHeaderLogout.addEventListener("click", () => {
      ...
      updateHeaderForUser();
      renderProducts();
      showToast("VocÃƒÆ’Ã‚Âª saiu da sua conta.");
    });
  }
  ...
  function createProductCardHTML(prod) {
    ...
    if (prod.category === "pre-venda") {
      ...
      badgeText = "PrÃƒÆ’Ã‚Â©-Venda";
      buttonLabel = "Adicionar ao Carrinho";
    }

    if (isFree) {
      badgeBg = "#25a244"; badgeText = "GrÃƒÆ’Ã‚Â¡tis";
      buttonLabel = "Download GrÃƒÆ’Ã‚Â¡tis"; buyClass += " free-btn";
    } else if (isPurchased) {
      badgeBg = "#25a244"; badgeText = "Adquirido";
      buttonLabel = prod.category === "pre-venda" && (!prod.downloadLink || prod.downloadLink.trim() === "") ? "Aguardando LanÃƒÆ’Ã‚Â§amento" : "Baixar Biblioteca"; 
      buyClass += " download-btn";
      priceStyle = "#52b788";
    }

    const priceText = isFree ? "GrÃƒÆ’Ã‚Â¡tis" : formatBRL(prod.price);
    ...
    card.innerHTML = `
      ...
      <div class="product-details">
        ...
        <div class="product-sales">
          <div class="sales-row">
            ${prod.category === "pre-venda" && prod.releaseDate ? `<div class="rating-score" style="color:#f59e0b;font-size:11px;">LanÃƒÆ’Ã‚Â§amento: ${prod.releaseDate}</div>` : `<div class="rating-score">ÃƒÂ¢Ã‹Å“Ã¢â‚¬Â¦ <span>${prod.rating.toFixed(1)}</span></div>`}
        ...
        <div class="product-actions">
          ...
          ${prod.youtube ? `<a href="${prod.youtube}" target="_blank" class="btn-secondary">Ver VÃƒÆ’Ã‚Â­deo</a>` : ""}
        </div>
      </div>`;
    return card;
  }
    ...
    container.querySelectorAll(".product-buy-btn").forEach(btn => {
      btn.addEventListener("click", e => {
        ...
        if (btn.classList.contains("free-btn") || btn.classList.contains("download-btn")) {
          ...
          if (itemExists) {
            showToast("Este item jÃƒÆ’Ã‚Â¡ estÃƒÆ’Ã‚Â¡ no carrinho!");
          } else {
            cart.push(prod);
            saveCart();
  ...
  document.getElementById("btnGoToYoutube")?.addEventListener("click", () => {
    ...
    showToast("Obrigado por se inscrever! Liberando download... ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Â°");
  ...
  function triggerDownload(prod) {
    ...
    if (!isLoggedIn()) {
      showToast("Cadastre-se ou faÃƒÆ’Ã‚Â§a login gratuitamente para baixar!");
      openAuthModal(registerModal);
      return;
    }

    // 2. Exigir inscriÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o no YouTube para libs GrÃƒÆ’Ã‚Â¡tis
    if (prod.category === "GrÃƒÆ’Ã‚Â¡tis" && localStorage.getItem("dawload_yt_sub") !== "true") {
    ...
    // 3. Efetuar download ou barrar prÃƒÆ’Ã‚Â©-venda
    if (prod.category === "pre-venda" && (!prod.downloadLink || prod.downloadLink.trim() === "")) {
      showToast(`Ainda em prÃƒÆ’Ã‚Â©-venda! DisponÃƒÆ’Ã‚Â­vel a partir de: ${prod.releaseDate || 'Breve'}`);
      return;
    }
    ...
    if (prod.downloadLink && prod.downloadLink.trim()) {
      ...
      showToast("Link de download nÃƒÆ’Ã‚Â£o configurado. Acesse o Painel Admin.");
    }
  }
  ...
  function renderCombos() {
    ...
    combosData.forEach(combo => {
      ...
      if (combo.features && combo.features.length) {
        featuresHtml = `<ul class="plan-features">` + combo.features.map(f => `<li>ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ ${f}</li>`).join('') + `</ul>`;
      }
    ...
    document.querySelectorAll(".combo-buy-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        if (itemExists) {
            showToast("Este combo jÃƒÆ’Ã‚Â¡ estÃƒÆ’Ã‚Â¡ no carrinho!");
        } else {
            cart.push(comboProd);
            saveCart();
  ...
  // Chamar logo na inicializaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o
  renderCombos();
  // ==========================================================
  //  BUSCA & SUGESTÃƒÆ’Ã¢â‚¬Â¢ES
  // ==========================================================
  ...
  function showSuggestions() {
    ...
    suggestionsDropdown.innerHTML = currentSuggestions.map((term, i) =>
      `<button class="suggestion-option" type="button" data-index="${i}"><span>ÃƒÂ¢Ã…â€™Ã¢â‚¬Â¢</span><span>${highlightSuggestion(term, query)}</span></button>`
    ).join("");
  ...
  // ==========================================================
  //  CHECKOUT ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â 3 Abas: PIX / CrÃƒÆ’Ã‚Â©dito / DÃƒÆ’Ã‚Â©bito
  // ==========================================================
  // ----------------------------------------------------------------------
  // configuraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o MERCADO PAGO - CARTÃƒÆ’Ã¢â‚¬Â¢ES (FRONTEND)
  // ----------------------------------------------------------------------
  // IMPORTANTE: Substitua pela sua PUBLIC KEY (Chave PÃƒÆ’Ã‚Âºblica) do Mercado Pago
  ...
  // Agora o frontend e backend rodam no mesmo domÃƒÆ’Ã‚Â­nio do Render!
  ...
  async function startPixCountdown() {
    ...
    try {
      ...
      if (data.success) {
        ...
        // 2. Iniciar Polling (perguntar ao servidor a cada 3s se jÃƒÆ’Ã‚Â¡ pagou)
        pollPaymentStatus();
      } else {
        const pixKeyEl = document.getElementById("pixKeyDisplay");
  ...
  function pollPaymentStatus() {
    ...
    pixAutoSyncTimer = setTimeout(async () => {
      if (!selectedProductForCheckout || !document.getElementById("tab-pix")?.classList.contains("active")) {
        return; // UsuÃƒÆ’Ã‚Â¡rio fechou ou trocou de aba
      }
  ...
  function detectBrand(num) {
    ...
    if (/^4/.test(n)) return { label: "VISA", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â³", css: "brand-visa" };
    if (/^5[1-5]/.test(n) || /^2[2-7]/.test(n)) return { label: "MC", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â´ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¡", css: "brand-mastercard" };
    if (/^6(36368|36369|36370|504175|362|38|09)/.test(n)) return { label: "ELO", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã¢â‚¬Âº", css: "brand-elo" };
    if (/^(384|385|386|368)/.test(n)) return { label: "HIPER", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â´", css: "brand-hipercard" };
    if (/^3[47]/.test(n)) return { label: "AMEX", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â·", css: "brand-amex" };
    return { label: "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â³", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â³", css: "" };
  }
  ...
  function setupCardInputs(prefix, card3dId) {
    ...
    numInput?.addEventListener("input", e => {
      ...
      const display = masked || "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢";
      if (numDisplay) numDisplay.textContent = display.padEnd(19, "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢").replace(/(.{4})/g, "$1 ").trim();
    ...
    cvvInput?.addEventListener("input", e => {
      ...
      e.target.value = v;
      if (cvvDisplay) cvvDisplay.textContent = v ? "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢".repeat(v.length) : "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢";
    });
  }
  ...
  function setupInstallments(price) {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      opt.value = i;
      opt.textContent = i === 1
        ? `1x de ${formatBRL(price)} (ÃƒÆ’Ã‚Â  vista)`
  ...
  function openCheckout(prod) {
    ...
    // Require login
    if (!isLoggedIn()) {
      showToast("FaÃƒÆ’Ã‚Â§a login ou cadastre-se para finalizar a compra.");
      openAuthModal(registerModal);
      return;
    }
  ...
  async function processPayment(method, installments, cardData) {
    ...
    if (rawCpf.length !== 11) {
      showToast("CPF invÃƒÆ’Ã‚Â¡lido! Por favor digite corretamente.", 4000);
      checkoutBody.style.display = "";
      checkoutProcessing.style.display = "none";
      return;
    }
    ...
    try {
      // 2. Tokenizar CartÃƒÆ’Ã‚Â£o de CrÃƒÆ’Ã‚Â©dito/DÃƒÆ’Ã‚Â©bito direto na API do Mercado Pago
      ...
      if (!tokenResponse || !tokenResponse.id) {
        ...
        throw new Error(tokenResponse.message || "NÃƒÆ’Ã‚Â£o foi possÃƒÆ’Ã‚Â­vel gerar o Token do CartÃƒÆ’Ã‚Â£o. Verifique os dados.");
      }
      ...
      const response = await fetch(`${API_BASE}/api/pay-card`, {
        ...
        body: JSON.stringify({
          ...
          paymentMethodId: method === "DÃƒÆ’Ã‚Â©bito" ? "debmaster" : "master", // SerÃƒÆ’Ã‚Â¡ descoberto pelo backend/SDK, mas enviamos fallback
          payer: {
      ...
      console.error("Erro ao processar cartÃƒÆ’Ã‚Â£o:", err);
      showToast(err.message || "Erro ao processar o cartÃƒÆ’Ã‚Â£o. Tente novamente.", 4000);
      checkoutBody.style.display = "";
      checkoutProcessing.style.display = "none";
    }
  }
  ...
  document.getElementById("btnConfirmCredit")?.addEventListener("click", () => {
    ...
    if (!cardData.number || !cardData.exp || !cardData.cvv || !cardData.name || !cardData.cpf || !cardData.email) {
      return showToast("Preencha todos os campos do cartÃƒÆ’Ã‚Â£o (inclusive CPF e E-mail).", 3000);
    }
    
    processPayment("CartÃƒÆ’Ã‚Â£o de CrÃƒÆ’Ã‚Â©dito", installments, cardData);
  });
  ...
  document.getElementById("btnConfirmDebit")?.addEventListener("click", () => {
    ...
    processPayment("DÃƒÆ’Ã‚Â©bito", 1, cardData);
  });
  ...
  // ==========================================================
  //  MINHA CONTA ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Painel com abas
  // ==========================================================
  function openMyAccount() {
    if (!myAccountOverlay) return;
    const user = getCurrentUser();
    const emailEl = document.getElementById("accountUserEmail");
    if (emailEl) emailEl.textContent = user ? user.email : "Downloads DisponÃƒÆ’Ã‚Â­veis";
    updateMyAccountList();
  ...
  function updateMyAccountList() {
    ...
    const purchases = getPurchasedIds();
    const myLibs = productsData.filter(p => p.category === "GrÃƒÆ’Ã‚Â¡tis" || purchases.includes(p.id));

    if (!myLibs.length) {
      myAccountDownloadsList.innerHTML = `<div style="text-align:center;padding:24px;color:var(--text-muted);font-size:13px;">Nenhum download disponÃƒÆ’Ã‚Â­vel ainda.</div>`;
      return;
    }
    ...
    myLibs.forEach(prod => {
      ...
      div.innerHTML = `
        ...
        <div class="purchase-history-info">
          ...
          <div class="purchase-history-meta" style="color:${prod.category==='GrÃƒÆ’Ã‚Â¡tis'?'#52b788':'var(--accent)'};">${prod.category==='GrÃƒÆ’Ã‚Â¡tis'?'GrÃƒÆ’Ã‚Â¡tis':'Adquirida'}</div>
        </div>
        <button type="button" class="btn-dl-small acc-dl-btn" data-id="${prod.id}">ÃƒÂ¢Ã‚Â¬Ã¢â‚¬Â¡ Download</button>`;
    ...
    [...purchases].reverse().forEach(p => {
      ...
      div.innerHTML = `
        <div class="purchase-history-info">
          ...
          <div class="purchase-history-meta">${date} Ãƒâ€šÃ‚Â· ${p.paymentMethod} Ãƒâ€šÃ‚Â· ${formatBRL(p.amount)}</div>
        </div>`;
    ...
    if (user.isAffiliate) {
      ...
      if (convList) {
        if (!stats.conversions.length) {
          convList.innerHTML = `<div style="text-align:center;padding:12px;color:var(--text-muted);font-size:12px;">Nenhuma venda atribuÃƒÆ’Ã‚Â­da ainda.</div>`;
        } else {
          convList.innerHTML = stats.conversions.map(c => `
            <div class="conversion-item">
              <span>${new Date(c.date).toLocaleDateString("pt-BR")} Ãƒâ€šÃ‚Â· ${c.productTitle}</span>
  ...
  if (heroCarousel && heroProducts.length > 0) {
    ...
    heroProducts.forEach((prod, index) => {
      ...
      // configuraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o para caixas de produtos verticais (3 camadas)
      ...
      if (prod.demoUrl || prod.youtubeUrl) {
        ...
        demoBtn = `<a href="#storeGrid" class="hero-btn">Ver CatÃƒÆ’Ã‚Â¡logo</a>`;
      }
  ...
  function renderHumanSupportCard(userQuery) {
    const defaultText = userQuery ? `OlÃƒÆ’Ã‚Â¡, preciso de suporte no D.A.W.LOAD: ${userQuery}` : "OlÃƒÆ’Ã‚Â¡, preciso de ajuda no D.A.W.LOAD";
    ...
    return `
      <div class="human-support-card">
        ...
        <a href="${rafaelLink}" target="_blank" class="human-contact-btn rafael">
          <span>ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬</span> Falar com Rafael (+55 21 981134378)
        </a>
        <a href="${alexLink}" target="_blank" class="human-contact-btn alex">
          <span>ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬</span> Falar com Alex (+55 13 99129-8707)
        </a>
      </div>`;
  }
  ...
  function processSupportTopic(topic) {
    ...
    if (topic === "download") {
      userText = "ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¥ Tenho problemas no download";
      botReply = `Se o seu download nÃƒÆ’Ã‚Â£o iniciou ou foi interrompido, verifique:<br>
      ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Se a sua conexÃƒÆ’Ã‚Â£o com a internet estÃƒÆ’Ã‚Â¡ estÃƒÆ’Ã‚Â¡vel.<br>
      ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Se o seu navegador nÃƒÆ’Ã‚Â£o bloqueou a janela pop-up do download.<br>
      ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Para bibliotecas GrÃƒÆ’Ã‚Â¡tis, certifique-se de estar logado no site e inscrito no canal do YouTube.<br><br>
      ...
      userText = "ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¹ DÃƒÆ’Ã‚Âºvida sobre instalaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o / Kontakt";
      botReply = `Nossas bibliotecas sÃƒÆ’Ã‚Â£o compatÃƒÆ’Ã‚Â­veis com **Kontakt Full (6 ou 7)** no Windows e macOS.<br><br>
      <strong>Passo a passo rÃƒÆ’Ã‚Â¡pido:</strong><br>
      ...
      Precisa de auxÃƒÆ’Ã‚Â­lio direto na instalaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o ou configuraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o?
      ${renderHumanSupportCard("Preciso de ajuda na instalaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o/configuraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o do Kontakt")}`;
    } else if (topic === "pagamento") {
      userText = "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â³ DÃƒÆ’Ã‚Âºvidas de Pagamento / Comprovante";
      botReply = `Pagamentos por **PIX** e **CartÃƒÆ’Ã‚Â£o** sÃƒÆ’Ã‚Â£o verificados e liberados no site.<br><br>
      Se vocÃƒÆ’Ã‚Âª efetuou o pagamento e deseja enviar seu comprovante ou tirar uma dÃƒÆ’Ã‚Âºvida de cobranÃƒÆ’Ã‚Â§a:
      ${renderHumanSupportCard("Gostaria de enviar o comprovante do meu pagamento no D.A.W.LOAD")}`;
    } else if (topic === "humano") {
      userText = "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ Quero falar com suporte humano";
      botReply = `Com certeza! Escolha abaixo com quem prefere falar diretamente no WhatsApp:
      ${renderHumanSupportCard("OlÃƒÆ’Ã‚Â¡! Gostaria de falar com o suporte do D.A.W.LOAD.")}`;
    }
  ...
  // Clique nos botÃƒÆ’Ã‚Âµes de tÃƒÆ’Ã‚Â³picos
  ...
  // FormulÃƒÆ’Ã‚Â¡rio de mensagem de texto livre
  supportChatForm?.addEventListener("submit", e => {
    ...
    if (norm.includes("download") || norm.includes("baixar") || norm.includes("link")) {
      reply = `Se vocÃƒÆ’Ã‚Âª nÃƒÆ’Ã‚Â£o conseguiu baixar, certifique-se de estar logado no site e ter liberado o link.<br><br>Para ajuda direta dos desenvolvedores:
      ${renderHumanSupportCard(text)}`;
    } else if (norm.includes("kontakt") || norm.includes("instalar") || norm.includes("erro") || norm.includes("nki")) {
      reply = `As bibliotecas exigem Kontakt 6 ou 7 Full.<br><br>Quer que ajudemos vocÃƒÆ’Ã‚Âª a configurar no seu computador?
      ${renderHumanSupportCard(text)}`;
    } else if (norm.includes("pix") || norm.includes("pagar") || norm.includes("comprar") || norm.includes("comprovante")) {
      reply = `Encaminhando seu comprovante / dÃƒÆ’Ã‚Âºvida de pagamento para verificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o rÃƒÆ’Ã‚Â¡pida:
      ${renderHumanSupportCard(text)}`;
    } else {
      reply = `Entendi a sua solicitaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o! Para te responder com atenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o total, escolha quem vocÃƒÆ’Ã‚Âª deseja chamar no WhatsApp:
      ${renderHumanSupportCard(text)}`;
    }
